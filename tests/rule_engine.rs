...
fn percentage_operators_compute_change() {
    ...
    assert!(matches!(
        evaluate(
            &rule(Operator::PctChangeDown, 10.0, Some(100.0)),
            &Sample {
                value: 85.0,
    ...
    assert_eq!(
        evaluate(
            &rule(Operator::PctChangeUp, 20.0, Some(100.0)),
            &Sample {
                value: 110.0,
                reference: Some(100.0)
            }
